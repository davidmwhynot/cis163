...
/***********************************************************************
 * Implementation of a StopWatch for project 1 in CIS 163. Tracks time in
 * minutes, seconds, and milliseconds.
 * 
 * @author David Whynot, Daniel Pfahler
 * @version 1.1.0
 ***********************************************************************/
...
public class StopWatch {
	...
	/**
	 * state of the stopwatch. if true, mutation of the above fields is not allowed!
	 */
	...
	/*******************************************************************
	 * Creates a stopwatch with given minutes, seconds, and milliseconds values.
	 * 
	 * @author David Whynot
	 * @param min
	 * @param sec
	...
	private void tryCreate(int min, int sec, int ms) {
		...
		/*
		 * set the fields to the given values if they are within the proper boundaries
		 */
		if (0 <= ms && ms <= 999) {
			milliseconds = ms;
		} else {
	 ...
	 * @author David Whynot
	 * @param ms
	 * @return array with elements 0, 1, and 2 being mins, secs, and ms respectively
	 *******************************************************************/
	...
	/*******************************************************************
	 * Adds the "this" StopWatch object.
	 * 
	 * @author David Whynot
	 * @param sw
	 *******************************************************************/
	...
	/*******************************************************************
	 * Subtracts the “this” StopWatch object.
	 * 
	 * @author David Whynot
	 * @param sw
	 *******************************************************************/
	public void sub(StopWatch sw) {
		if (!suspended) {
			int newMS = convertDown(minutes, seconds, milliseconds)
					- convertDown(sw.getMinutes(), sw.getSeconds(), sw.getMilliseconds());
			if (newMS >= 0) {
				int[] temp = convertUp(newMS);
				minutes = temp[0];
				seconds = temp[1];
	...
	public boolean equals(Object o) {
		if (o != null && o instanceof StopWatch) {
			return (((StopWatch) o).getMinutes() == minutes) && (((StopWatch) o).getSeconds() == seconds)
	...
	public static boolean equals(Object sw1, Object sw2) {
		if ((sw1 != null) && (sw2 != null) && (sw1 instanceof StopWatch) && (sw2 instanceof StopWatch)) {
			return (((StopWatch) sw1).getMinutes() == ((StopWatch) sw2).getMinutes())
					&& (((StopWatch) sw1).getSeconds() == ((StopWatch) sw2).getSeconds())
					&& (((StopWatch) sw1).getMilliseconds() == ((StopWatch) sw2).getMilliseconds());
		} else {
			throw new IllegalArgumentException();
		}
	}
	...
	public int compareTo(Object o) {
		if (o != null && o instanceof StopWatch) {
			int otherMS = convertDown(((StopWatch) o).getMinutes(), ((StopWatch) o).getSeconds(),
